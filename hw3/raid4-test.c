...
int main() {
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 3; j++) {
            ...
            nblks = blkdev_num_blocks(raid4);

            // reports the correct size
            assert(raid4 != NULL);
            assert(nblks == blkdev_num_blocks(disks[0]) / unit * unit * (ndisk - 1));

            // read/write cross stripe of the raid4
            ...
            assert(blkdev_write(raid4, 0, nblks, backup) == SUCCESS);
            ...
            image_fail(newdisk);
            image_fail(disks[1]);
            assert(blkdev_write(raid4, 0, 1, write_buf) != SUCCESS);
            assert(blkdev_read(raid4, 0, 1, read_buf) != SUCCESS);
